  ...
  run(trainer) {
    ...
    if (typeof userFound !== "undefined") {
      Trainers.update(
        ...
        { $push: { education: trainer.education[0] } }
      );
      return true;
    } else {
...
export const upsertEmploymentTrainer = new ValidatedMethod({
  ...
  run(trainer) {
    ...
    if (typeof userFound !== "undefined") {
      Trainers.update(
        { idUser: Meteor.userId() },
        { $push: { employment: trainer.employment[0] } }
      );
      return true;
    } else {
      browserHistory.push(`/trainer/edit/experience`);
...
export const removeTrainerEducation = new ValidatedMethod({
  ...
  run({ schoolToDelete }) {
    Trainers.update(
      ...
      {
        $pull: {
          education: { school: schoolToDelete }
        }
      }
    );
  }
...
export const removeTrainerEmployment = new ValidatedMethod({
  ...
  run({ companyToDelete }) {
    Trainers.update(
      ...
      {
        $pull: {
          employment: { company: companyToDelete }
        }
      }
    );
  }
